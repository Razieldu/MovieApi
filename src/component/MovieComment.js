    ...
    const getComment = async () => {
      const getMovieComment = async () => {
        const response = await fetch(
          `https://api.themoviedb.org/3/movie/${param.movieId}/reviews?api_key=${api}&language=en-US&page=1`
        );
        if (!response.ok) {
      ...
      const getMovieImage = async () => {
        const response = await fetch(
          `https://api.themoviedb.org/3/movie/${param.movieId}/images?api_key=${api}`
        );
        if (!response.ok) {
          throw new Error("can not get data");
        }
